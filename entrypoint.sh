...
if ! flyctl status --app "$app"; then
  ...
  flyctl deploy --app "$app" --regions "$region" --image "$image" --regions "$region" --strategy immediate
elif [ "$INPUT_UPDATE" != "false" ]; then
  flyctl deploy --config "$config" --app "$app" --regions "$region" --image "$image" --regions "$region" --strategy immediate
fi
